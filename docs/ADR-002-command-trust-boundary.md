...
- Offline/outbox command vẫn có `commandId` và `issuedAt` để idempotency/sync nhưng không trở thành security authority.

### Trade-off

Offline client có thể cần giữ local display metadata về actor/device/store để UX hoặc diagnostics, nhưng metadata đó phải nằm ngoài authoritative command identity và luôn được server đối chiếu/enrich khi sync.
...
5. Event/audit record dùng server-enriched identity.
6. Nếu business payload thật sự cần tham chiếu một entity Store-scoped, entity đó vẫn phải được kiểm tra thuộc trusted Store trước mutation.
